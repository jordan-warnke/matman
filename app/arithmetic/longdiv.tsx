...
function formatProblemPrompt(p: LDProblem): string {
  return p.displayMode === 'divide'
    ? `${p.dividend} ÷ ${p.divisor}`
    : `${p.a} × ${p.b}`;
}

function formatOptionHint(p: LDProblem, option: number): string | null {
  if (p.displayMode === 'multiply') {
    const divisors: number[] = [];
    if (option % p.a === 0) divisors.push(p.a);
    if (p.b !== p.a && option % p.b === 0) divisors.push(p.b);
    if (divisors.length === 0) {
      return p.a === p.b
        ? `(not divisible by ${p.a}!)`
        : `(not divisible by ${p.a} or ${p.b}!)`;
    }
    const preferred = divisors.includes(p.a) ? p.a : divisors[0];
    return `(${preferred} × ${option / preferred})`;
  }

  if (p.displayMode === 'divide' && p.dividend && p.divisor) {
    if (option !== 0 && p.dividend % option === 0) {
      return `(${p.dividend} ÷ ${p.dividend / option})`;
    }
    return `(${option} doesn't divide evenly into ${p.dividend})`;
  }

  return null;
}

export default function LongDivGameScreen() {
  const router = useRouter();
  const { colors, timed, multipleChoice, isWork } = useTheme();
  const { type } = useLocalSearchParams<{ type: string }>();
  const gameType: GameType = (type || 'longdiv-drill') as GameType;
  ...
  // ── problem generator ──
  const { generate, record, reshuffleOptions } = useLongDivGenerator(
    settings.operandMode,
  ...
  // ── game state ──
  const [problem, setProblem] = useState<LDProblem | null>(null);
  ...
  const animateSelectedOption = useCallback((isCorrect: boolean) => {
    ...
    if (isCorrect) {
      Animated.sequence([
        Animated.timing(selectedOptionScale, { toValue: 1.035, duration: 110, useNativeDriver: true }),
        Animated.spring(selectedOptionScale, { toValue: 1, friction: 7, tension: 120, useNativeDriver: true }),
      ]).start();
      return;
    }
    Animated.sequence([
    ...
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]}>
        <View style={styles.center}>
          ...
          <TouchableOpacity style={styles.secondaryBtn} activeOpacity={0.7} onPress={() => router.back()}>
...
const styles = StyleSheet.create({
  ...
  timerPill: { paddingHorizontal: 18, paddingVertical: 6, borderRadius: 20, borderWidth: 2 },
  timerText: { ...Font.h3 },
  ...
  problemText: { fontSize: 44, fontWeight: '800' },
  inlineReveal: { fontSize: 40, fontWeight: '800', textAlign: 'center', marginTop: Spacing.xs },
  reviewPill: { marginTop: Spacing.sm, paddingHorizontal: 12, paddingVertical: 5, borderRadius: 999, borderWidth: 1 },
  reviewPillText: { ...Font.caption, fontWeight: '700', letterSpacing: 0.3, textTransform: 'uppercase' },

  answerArea: { paddingHorizontal: Spacing.xl, paddingBottom: Spacing.md },
  inputRow: { flexDirection: 'row', alignItems: 'center', gap: Spacing.sm },
  display: { flex: 1, borderWidth: 2, borderRadius: 16, padding: Spacing.md, alignItems: 'center', justifyContent: 'center', minHeight: 56 },
  displayText: { fontSize: 24, fontWeight: '700', textAlign: 'center' },

  nextBtn: { height: 64, borderRadius: 20, justifyContent: 'center', alignItems: 'center', borderBottomWidth: 4 },
  nextText: { fontSize: 28, fontWeight: '800', color: '#FFF' },
  navRow: { flexDirection: 'row', gap: Spacing.sm, marginTop: Spacing.md },
  navRowHidden: { opacity: 0 },
  ...
  mcGrid: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: Spacing.sm },
  mcBtnWrap: { width: '45%' },
  mcBtn: { width: '100%', borderRadius: 16, paddingVertical: 18, alignItems: 'center', borderWidth: 2, borderBottomWidth: 4 },
  mcText: { ...Font.h2 },
  mcHintText: { ...Font.caption, marginTop: 4, opacity: 0.85 },
  optionKeyPill: {
    position: 'absolute', top: 10, right: 10, minWidth: 24,
    paddingHorizontal: 6, paddingVertical: 2, borderRadius: 999, borderWidth: 1,
    alignItems: 'center', justifyContent: 'center',
  },
  optionKeyText: { ...Font.caption, fontWeight: '700', fontVariant: ['tabular-nums'] },
  ...
  primaryBtn: { paddingHorizontal: Spacing.xl, paddingVertical: 16, borderRadius: 16, borderBottomWidth: 4 },
