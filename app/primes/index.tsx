...
const GAME_TYPE: GameType = 'primes-time-attack';
const PRIMES_DEFAULTS: ModeSettings = {
  ...DEFAULT_MODE_SETTINGS,
  maxNumber: 200,
};
...
export default function PrimesHub() {
  ...
  useEffect(() => {
    loadModeSettings(GAME_TYPE).then((s) =>
      setSettings({ ...PRIMES_DEFAULTS, ...s }),
    );
  }, []);
  ...
  function commitDrafts() {
    const maxNumber = clamp(drafts.maxNumber, 20, 200);
    ...
    patch({ maxNumber, timeAttackSeconds, problemCount });
    setDrafts({
      ...
      <ScrollView contentContainerStyle={styles.body}>
        ...
        <View style={[styles.modeCard, { borderColor: colors.primary, backgroundColor: colors.card }]}>
          <TouchableOpacity
            ...
            activeOpacity={0.85}
            onPress={() => router.push({ pathname: '/primes/game', params: { type: GAME_TYPE } })}
          >
        ...
        <View style={styles.overlay}>
          <View style={[styles.modal, { backgroundColor: colors.card }]}>
            ...
            <View style={styles.modalRow}>
              <Text style={[styles.modalLabel, { color: colors.text }]}>Max number (20–200)</Text>
              <TextInput
                ...
                onBlur={() => {
                  const c = clamp(drafts.maxNumber, 20, 200);
